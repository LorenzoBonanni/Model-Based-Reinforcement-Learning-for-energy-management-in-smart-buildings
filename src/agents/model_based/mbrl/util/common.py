...
def calc_rest_ensemble_mean_std_leave_out_model_indices(means_of_all_ensembles: torch.Tensor,
                                                        ...
                                                        model_indices: torch.Tensor, ensemble_size: int) \
        -> Tuple[torch.Tensor, torch.Tensor]:
    """Generates the mean and Covariance(diagonal) of the merged remaining gaussian. model_indices states the gaussian,
    which was already used for prediction. It should not condidered for the ensemble vote of the rest. In other words
    the function calculates the mean and Covariance of the gaussian which results of the ensemble vote of the not picked
    gaussians. Formula is used from the M2AC Paper and can be found as Screenshot
    in docs/resources/formulas/formula_calc_rest_mean_std.png

    Args:
        means_of_all_ensembles(torch.Tensor):  is [ensemble_size,model_input.shape[0](batchsize), observationsize+1] Tensor with
        the mean of every(ensemble_size many) MLP for each observation action pair
        stds_of_all_ensembles(torch.Tensor):  is [ensemble_size,model_input.shape[0](batchsize), observationsize+1] Tensor with
        the stds of every(ensemble_size many) MLP for each observation action pair
        model_indices (torch.Tensor): [model_input.shape[0]] sized Tensor with indices for whiched model is picked from
        [0,ensemble_size)
        ensemble_size (int): ensemble size of Gaussian MLP Ensemble
    Returns:
        Tuple[means_of_rest_ensembles, var_of_rest_ensembles]
    """
    means_of_rest_ensembles = means_of_all_ensembles.detach().clone()
    stds_of_rest_ensemble = stds_of_all_ensembles.detach().clone()
    iter_list = torch.Tensor(range(model_indices.size(dim=0))).long()

    means_of_rest_ensembles[model_indices, iter_list, :] = 0
    stds_of_rest_ensemble[model_indices, iter_list, :] = 0
    return calc_rest_ensemble_mean_std(means_of_rest_ensembles, stds_of_rest_ensemble, ensemble_size - 1)


def calc_rest_ensemble_mean_std(means_of_all_ensembles: torch.Tensor,
                                stds_of_all_ensembles: torch.Tensor, count: int = 0) \
        -> Tuple[torch.Tensor, torch.Tensor]:
    """Generates the mean and Covariance(diagonal) of the merged remaining gaussian
    Formula is used from the M2AC Paper and can be found as Screenshot
    in docs/resources/formulas/formula_calc_rest_mean_std.png
    ...
    Args:
        ...
        stds_of_all_ensembles(torch.Tensor):  is [ensemble_size,model_input.shape[0](batchsize), observationsize+1] Tensor with
        the stds of every(ensemble_size many) MLP for each observation action pair
        count(int): the count_gaussian_to_merge
    Returns:
        Tuple[means_of_rest_ensembles, var_of_rest_ensembles]
    """
    if count == 0:
        count_gaussian_to_merge = means_of_all_ensembles.size(dim=0)
    else:
        count_gaussian_to_merge = count
    # calc mean
    means_of_rest_ensembles = means_of_all_ensembles.detach().clone()
    means_of_rest_ensembles = torch.sum(means_of_rest_ensembles, 0)
    means_of_rest_ensembles = torch.div(means_of_rest_ensembles, count_gaussian_to_merge)

    # calc std
    means_of_rest_ensembles_squared = means_of_all_ensembles.detach().clone()
    means_of_rest_ensembles_squared = torch.pow(means_of_rest_ensembles_squared, 2)
    means_of_rest_ensembles_squared = torch.sum(means_of_rest_ensembles_squared, 0)
    means_of_rest_ensembles_squared = torch.div(means_of_rest_ensembles_squared, count_gaussian_to_merge)

    stds_of_all_ensembles_squared = stds_of_all_ensembles.detach().clone()
    stds_of_all_ensembles_squared = torch.pow(stds_of_all_ensembles_squared, 2)
    stds_of_all_ensembles_squared = torch.sum(stds_of_all_ensembles_squared, 0)
    stds_of_all_ensembles_squared = torch.div(stds_of_all_ensembles_squared, count_gaussian_to_merge)
...
def evaluate_for_building(
    ...
    Uses agent to act in environemnt. Calculates the mean reward over the episodes.
    Also uses videorecorder to capture agents behaviour in environment.
    This version is made up for FMACURA
    Args:
        env (gym.Env): The environment of the evaluations
        num_episodes (int): Number of episodes to evaluate the agent
        agent (SACAgent): Agent to evaluate
    ...
    for episode in range(num_episodes):
        ...
        L = obs_full.shape[0]
        obs_dim_per_building = L // num_buildings
        terminated= truncated = False
        episode_reward = 0
        while not terminated and not truncated:
            obs_per_building = obs_full.reshape((num_buildings, obs_dim_per_building))

            actions_per_building: List[np.ndarray] = []
            for b in range(num_buildings):
                a_b = agents[b].act(obs_per_building[b])
                actions_per_building.append(np.asarray(a_b).reshape(-1))
            joint_action = np.concatenate(actions_per_building, axis=0)

            next_obs_full, reward, terminated, truncated, _ = env.step(joint_action)
...
def create_one_dim_tr_model(
    ...
    # Possible to process each observation before putting it into the model as input together with the action
...
def train_model_and_save_model_and_data(
    ...
    """
    # dataset_train is bootstrap iterator, so it gets seperate batches for each model to train.
    # dataset_val is a Transition iterator, because we only need to sample one batch out of the iterator
    # to validate if model ensemble is making good predicitons
    ...
    model_trainer.train(
        ...
        callback=callback,
        train_period=cfg.freq_train_model
    )
...
def rollout_agent_trajectories_Tracking_States(
...
def rollout_agent_trajectories_per_building(
        real_experienced_states_full: List[Any],
        env: gym.Env,
        steps_or_trials_to_collect: int,
        agents: List[mbrl.planning.Agent],
        agent_kwargs: Dict,
        trial_length: Optional[int] = None,
        callback: Optional[Callable] = None,
        replay_buffers: Optional[List[ReplayBuffer]] = None,
        collect_full_trajectories: bool = False
) -> List[float]:
    """
    Same behavior as before, but suppose that `env.reset()` and `env.step(...)`
    return a single 1-D "flattened" array containing all concatenated observation blocks. 
    For example: if L = len(obs_full) and there are N buildings,
    then L must be a multiple of N and each building has obs_dim = L//N features.

    This function automatically splits that vector into a matrix
    with shape=(num_buildings, obs_dim_per_building) before calling each agent[b].act(...) on obs[b].
    """


    num_buildings = len(agents)
    if replay_buffers is not None and len(replay_buffers) != num_buildings:
        raise ValueError(f"Se passi replay_buffers, devono essere {num_buildings}, non {len(replay_buffers)}.")
    ...
    while True:
        obs_full, info = env.reset()
        # How many feature per building:
        L = obs_full.shape[0]
        if L % num_buildings != 0:
            raise ValueError(f"Lunghezza osservazione ({L}) non multipla di num_buildings ({num_buildings}).")
        obs_dim_per_building = L // num_buildings
        obs = obs_full.reshape((num_buildings, obs_dim_per_building))

        for a in agents:
            a.reset()
        terminated = False
        truncated  = False
        total_reward_per_trial = 0.0

        while not terminated and not truncated:
            if replay_buffers is not None:
                real_experienced_states_full.append(
                    mbrl.util.mujoco.MujocoEnvHandler.get_current_state(env)
                )

            actions_per_building = []
            for b in range(num_buildings):
                obs_b = obs[b]  # array of hape=(obs_dim_per_building,)
                a_b = agents[b].act(obs_b, **agent_kwargs)
                actions_per_building.append(a_b)
            # joint action shape=(num_buildings, act_dim_per_building)
            joint_action = np.concatenate([a_b.reshape(-1) for a_b in actions_per_building], axis=0)
            next_obs_full, reward_full, terminated, truncated, info = env.step(joint_action)
            reward_full = np.array(reward_full, dtype=float)
            if next_obs_full.shape[0] != L:
                raise ValueError(
                    f"Lunghezza next_obs ({next_obs_full.shape[0]}) differisce da L ({L})."
                )
            next_obs = next_obs_full.reshape((num_buildings, obs_dim_per_building))

            # Updating replay buffers
            if replay_buffers is not None:
                for b in range(num_buildings):
                    replay_buffers[b].add(
                        obs[b].astype(np.float32),
                        actions_per_building[b].astype(np.float32),
                        next_obs[b].astype(np.float32),
                        reward_full[b],
                        terminated, 
                        truncated
                    )
            ...
            obs = next_obs
            total_reward_per_trial += reward_full.sum()
            step += 1

            if not collect_full_trajectories and step == steps_or_trials_to_collect:
                total_rewards.append(total_reward_per_trial)
                return total_rewards
            
            if trial_length and step % trial_length == 0:
                if collect_full_trajectories and replay_buffers is not None:
                    for b in range(num_buildings):
                        replay_buffers[b].close_trajectory()
                break
...
def step_env_and_add_to_buffer_per_building(
        env: gym.Env,
        obs: np.ndarray,
        agents: List[mbrl.planning.Agent],
        agent_kwargs: Dict,
    ...
    """Steps the environment with an agent's action and populates the replay buffer.
        This version is an adaption of the previous one but it works with Factorized Citylearn
    Args:
        env (gym.Env): the environment to step.
        obs (np.ndarray): the latest observation returned by the environment (used to obtain
            an action from the agent).
    ...
    Returns:
        ...
        `env.step(agent.act(obs))`.
    """
    # Separating the observation
    L = obs.shape[0]
    obs_dim_per_building = L // num_buildings
    obs_per_build = obs.reshape((num_buildings, obs_dim_per_building))
    ...
    for b in range(num_buildings):
        if agent_uses_low_dim_obs:
            low_dim = env.get_last_low_dim_obs()
            action_b = agents[b].act(low_dim, **agent_kwargs)
        else:
            action_b = agents[b].act(obs_per_build[b], **agent_kwargs)
        actions_per_building.append(np.asarray(action_b).reshape(-1))
    ...
    next_obs_per_build = next_obs.reshape((num_buildings, obs_dim_per_building))
    for b in range(num_buildings):
        replay_buffers[b].add(obs_per_build[b],actions_per_building[b],next_obs_per_build[b], reward[b], terminated, truncated)
    return next_obs, reward, terminated, truncated, info
