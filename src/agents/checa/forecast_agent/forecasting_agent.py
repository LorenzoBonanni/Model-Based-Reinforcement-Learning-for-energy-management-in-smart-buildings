import numpy as np
from agents.checa.forecast_agent.ts_forecast_ensemble import TimeSeriesEnsemble
from agents.checa.forecast_agent.utils import fast_closest_hour
...
class ForecastAgent:
    ...
    def save_data_and_get_cur_states(self, observations):
        ...
        direct_sol_6h = observations[self.observation_names_b.index("direct_solar_irradiance_predicted_1")]
        diff_sol_6h = observations[self.observation_names_b.index("diffuse_solar_irradiance_predicted_1")]
        direct_sol_12h = observations[self.observation_names_b.index("direct_solar_irradiance_predicted_2")]
        diff_sol_12h = observations[self.observation_names_b.index("diffuse_solar_irradiance_predicted_2")]
        direct_sol_24h = observations[self.observation_names_b.index("direct_solar_irradiance_predicted_3")]
        diff_sol_24h = observations[self.observation_names_b.index("diffuse_solar_irradiance_predicted_3")]
